...
void Guitar::transpose_chord_up(string chord, int number_transpose_up)
{
	bool found = false;
tail = head;
while(tail!=NULL)	//loop through the chord
{
	...
	if(found == false)
	{
		cout << chord << " does not exist! Please enter another chord to transpose up!" << endl;
	}

}

//This is the transpose down function, which takes in the chord as a string and the number to traspose up down an int
void Guitar:: transpose_chord_down(string chord, int number_transpose_down)
{
bool found = false;
tail = head;
while(tail!=NULL)
{
	...
	if(found == false)
	{
		cout << chord << " does not exist! Please enter another chord to transpose down!" << endl;
	}
...
void Guitar::display_chord()
{
	if(head == NULL)
	{
		cout << "All chords have been cleared. Please build list first" << endl; // If the chord is empty, the user is notified to build first
	}
	else
	{
	tail = head;
